...
const SIGNAL_COLOR = { r: 127, g: 219, b: 202 };
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < CONNECTION_DISTANCE) {
            ...
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.strokeStyle = `rgba(${SIGNAL_COLOR.r}, ${SIGNAL_COLOR.g}, ${SIGNAL_COLOR.b}, ${alpha})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        }
      ...
      for (let i = 0; i < particles.length; i += 3) {
        ...
        if (dAB < CONNECTION_DISTANCE && dBC < CONNECTION_DISTANCE && dCA < CONNECTION_DISTANCE) {
          ...
          ctx.lineTo(c.x, c.y);
          ctx.closePath();
          ctx.fillStyle = `rgba(${SIGNAL_COLOR.r}, ${SIGNAL_COLOR.g}, ${SIGNAL_COLOR.b}, ${alpha})`;
          ctx.fill();
        }
      }
      ...
      if (!isIdleRef.current) {
        ...
        cursorGradient.addColorStop(0, `rgba(${SIGNAL_COLOR.r}, ${SIGNAL_COLOR.g}, ${SIGNAL_COLOR.b}, 0.06)`);
        cursorGradient.addColorStop(1, 'rgba(127, 219, 202, 0)');
      ...
      ctx.fillStyle = `rgba(${SIGNAL_COLOR.r}, ${SIGNAL_COLOR.g}, ${SIGNAL_COLOR.b}, 0.7)`;
      ctx.fill();
