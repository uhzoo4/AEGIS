...
> [!NOTE]
> **v2 — patched.** Five fixes from review: §12 (AI Experience Strategy) rewritten with an actual scope decision instead of a wish list; §14.0 (Demo Floor) added — explicit P0/P1/P2 cut lines and live-presentation pacing; §15 cursor/canvas conflict decided now instead of deferred to "profile later"; §4/§6.1 Preserved-vs-Extends wording reconciled. Everything else is unchanged from v1.
...
| `WhySection` | Observe/Adapt/Protect cards | → Preserved exactly as built; only addition is a `<SectionIndex>` wrapper, no structural change |
| `TimelineSection` | Hackathon progress tracker | → Preserved exactly as built; `<SectionIndex>` wrapper only — a GSAP-driven progressive reveal is P1 polish, not worth touching a working component during Phase 1 |
...
## 12. AI Experience Strategy *(v2 — patched)*

> [!WARNING]
> This was the thinnest section in the original draft — four bullet rows for the one thing this hackathon actually mandates (Google AI Studio is the core tool, and finalists present live to judges). Replaced below with an actual scope decision, not a wish list.

### 12.1 The honest scope decision

Nothing in "What Exists Today" (§0) touches a real calendar, inbox, or task source — no OAuth, no backend, no data pipeline. Promising live Google Calendar/Gmail integration on top of everything else in this document, solo, in the time remaining, is not realistic, and pretending otherwise is how a hackathon project loses its last day to an integration that was never the point.

**The honest scope: AEGIS reasons over a seeded, hand-written "signal dataset"** — one plausible pressure scenario (calendar density, message backlog, two colliding threads, a tone shift) — not a live data source. Whether the input data is live or seeded is invisible to a judge watching a demo. Whether the *reasoning* is real or scripted is not — and that's the part that has to be genuinely real. Gemini's job is to reason about the seed data and produce the diagnosis, forecast, and intervention. That reasoning is the actual product being demonstrated.

### 12.2 One call, not three

The original component map (§6.1) implies three separate integration points — `SignalTrack`, `ForecastScene`, and `InterventionPreview` each independently calling Gemini. Don't build three integrations. **Build one Gemini call that returns one structured JSON object**, and have the three visual beats each render a different slice of that same response. One failure point instead of three during a live demo, and one coherent diagnosis instead of three disconnected features. Target shape (the shape matters more than the exact field names):

```json
{
  "signals": [
    { "label": "Calendar density", "detail": "...", "severity": "high" },
    { "label": "Message backlog", "detail": "...", "severity": "medium" }
  ],
  "forecast": { "collision_time": "14:40", "description": "..." },
  "intervention": { "action": "...", "rationale": "..." }
}
```

Use Gemini's native structured-output/JSON-mode constraint rather than parsing free text — this removes an entire category of "the model added a preamble sentence and broke my parser" bugs the night before a demo.

### 12.3 Model choice

Don't hand-write the API call from memory — model names and SDK syntax move fast. **Generate your starter snippet directly from Google AI Studio's "Get code" panel** after picking a model there, and adapt that, rather than trusting any snippet written from documentation or training data. A Flash-tier model (the lower-cost, lower-latency tier — check whichever current Flash or Flash-Lite option AI Studio offers on a free quota) is the right pick: this is structured extraction over a short document, not deep multi-step reasoning, so the Pro tier's cost and latency buys nothing here.

### 12.4 Demo-safety: precompute the default, keep live as a flex

The single highest-risk moment in a live, judged presentation is a network call you don't control. Structure it so the demo's critical path never depends on one:

1. **Generate 1–2 signal scenarios once, during development**, by actually calling Gemini and saving the response to a static file (`src/data/signal-default.json`). This *is* using Google AI Studio as part of the solution-building process — it's just not re-running inference live on every demo.
2. **The page loads with this precomputed response by default.** Zero network dependency on the critical path. This cannot fail live.
3. **Optionally, add a small "try your own situation" input** that fires a real, live Gemini call when used — your genuine, demoable AI capability, available on request, but not load-bearing for the core pitch.
4. **Any live call gets a hard ~5s timeout** that falls back to the cached default with no visible error. The audience should never see a spinner outlast their attention span.

### 12.5 Where the key lives

If you build the live "try your own" path and have 30–60 minutes to spare, put the Gemini call behind one small serverless function (a Vercel/Netlify function deploys free with this stack) so the API key never reaches the browser bundle. If you don't have that time, a client-side call with a key scoped to a low daily quota in AI Studio is an acceptable hackathon-grade fallback — just know that's what it is, and don't carry that pattern past the hackathon.

### 12.6 The "System Speaking" convention

Every AI-generated insight — including the loading state — displays in the **System register** (mono, uppercase, tracked, bracketed), never the Editorial register. While the response resolves (precomputed-instant by default, or live on `regenerate()`), show `[ SYS.AEGIS // ANALYZING ]` in the same register, not a generic spinner — the wait is in-character, not a UI gap:
...
### 14.0 Demo Floor — what ships no matter what *(read this before Phase 1)*

A solo build with no cut line is how a hackathon project loses a week to a feature nobody sees. Three tiers, decided now:

- **P0 — must work, no exceptions:** Gate (brief, ≤3s, skippable) → `<PressureHero>` (a strong masked headline reveal is an acceptable substitute if the scratch-reveal canvas isn't done) → the AI moment from §12, default-loaded, presented as **one held scene**, not three separate components → `<Manifesto>` close. This alone proves the thesis — observed → understood → guided → protected — and is the demo if everything else gets cut.
- **P1 — build if Phase 1–2 finish on schedule:** Split the single AI scene into the full `SignalTrack` → `ForecastScene` → `InterventionPreview` sequence with its own pinned/scrubbed motion. Magnetic CTAs. Scratch-reveal canvas replacing the plain masked reveal in the Pressure beat.
- **P2 — cut without remorse if time is short:** Scene-cycling hero (one strong reveal does the job). Section-aware cursor color-temperature shift. The "gate letters become the header logo" detail.

**Demo pacing for the live presentation round, specifically:** a scrolling cinematic experience is great for someone exploring on their own time; it's a liability in a timed, screen-shared pitch where scroll speed isn't fully in your control. Don't perform the full scroll live as the whole demo. Either (a) let the gate+hero scroll play for the opening few seconds only, then jump via an anchor/keyboard shortcut straight to the AI moment so you're not scrolling through transitions in real time while talking, or (b) keep the AI moment + Manifesto pre-loaded in a second tab ready to switch to. Decide which now, not five minutes before presenting.
...
- [ ] Scratch-reveal canvas interaction (Pressure beat) — P1; a masked headline reveal alone satisfies P0
- [ ] Wire `CursorField`'s RAF loop to pause while the scratch-reveal canvas is active, resume after (§15 — decided, not deferred; only one canvas runs at a time)
- [ ] Section-aware cursor color shifting — P2
...
> [!NOTE]
> All three components below consume slices of the **same** Gemini response from §12.2 — there is one API integration to get working, not three. Don't start this phase until `useSignalResponse()` (Track B in §12) is proven solid; building three components against an unstable data shape is how this phase overruns.
...
|---------|------|------------|
| `CursorField`'s 45-particle mesh + a new scratch-reveal canvas running simultaneously during the Pressure beat | Two independent RAF loops + two canvas contexts at once is a real frame-budget risk on mid-tier hardware — not a "profile later" question | **Decided in §14.0/Phase 2:** only one canvas runs at a time. `CursorField` pauses its loop while the scratch-reveal canvas is active and resumes after. Drive this off the same `SectionContext` already planned for cursor color-shifting — one more state read, not new plumbing |
...
| Live Gemini call during the demo | A network call you don't control, at the worst possible moment | §12.4 — precomputed default loads instantly; live call is opt-in via `regenerate()` with a 5s timeout, never the critical path |
